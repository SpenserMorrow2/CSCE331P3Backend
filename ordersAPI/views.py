...
def salesReportNumbers(start_date, end_date):
    ...
    sales_counts = OrderItems.objects.filter(orderid__range=[orderIDs[0], orderIDs[len(orderIDs)-1]]).annotate(count=Count('itemid'))
    ...
    for sale in sales_counts:
        item_sales[int(sale['itemid'])] = sale['count']

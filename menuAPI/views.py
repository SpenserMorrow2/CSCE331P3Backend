...
@api_view(['PATCH'])
def changeMenuName(request, itemid):
    new_name = request.data.get('name')
    try:
        menu_item = MenuItem.objects.get(itemid=itemid)
    except MenuItem.DoesNotExist:
        return Response(status=status.HTTP_404_NOT_FOUND)
    ...
    return Response({"message":f"Name for {menu_item.name} changed successfully"}, status=status.HTTP_200_OK)

@api_view(['PATCH'])
def changeMenuNutrition(request, itemid):
    new_cals = request.data.get('calories')
    new_serving = request.data.get('serving_size')

    #input validation
    errors = {}
    if not new_cals or not isinstance(new_cals, str) or len(new_cals) > 10:
        errors['calories'] = "Calories must be a non-empty string with a max length of 10."
    if not new_serving or not isinstance(new_serving, str) or len(new_serving) > 10:
        errors['serving_size'] = "Serving size must be a non-empty string with a max length of 10."

    if errors:
        return Response(errors, status=status.HTTP_400_BAD_REQUEST)

    try:
        menu_nutrition = MenuCalories.objects.get(itemid=itemid)
    except MenuCalories.DoesNotExist:
        return Response({"error": "Menu item not found."}, status=status.HTTP_404_NOT_FOUND)

    menu_nutrition.calories = new_cals
    menu_nutrition.serving_size = new_serving
    menu_nutrition.save()

    return Response({"message": f"Nutrition for item ID {menu_nutrition.itemid} changed successfully."}, status=status.HTTP_200_OK)
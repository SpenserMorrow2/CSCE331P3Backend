...
@api_view(['PATCH'])
def PromoteEmployees(request, format=None):
    #verify id exists
    promotedEmployees = request.data.get('names', [])
    try:
        for emp in promotedEmployees:
    ...
    return Response({"message":f"Employees has been promoted to Manager"}, status=status.HTTP_200_OK)

@api_view(['GET'])
def getAllActiveEmployeesNonManager(request, format=None):
    active_Employees_nonMan = Employee.objects.filter(employmentstatus='active').filter(type='Employee')
    serializer = EmployeeSerializer(active_Employees_nonMan, many=True)
    return Response(serializer.data)

@api_view(['GET'])
def getAllInActiveEmployees(request, format=None):
    active_Employees_nonMan = Employee.objects.filter(employmentstatus='inactive')
    serializer = EmployeeSerializer(active_Employees_nonMan, many=True)
    return Response(serializer.data)